...
# Code to read and merge student-por.csv and student-mat.csv
...
> **NOTE:** Refer Appendix 1 for summary of full addtive model 
...
summary_intMdl1 = summary(student_performance_intMdl1)
```

&nbsp;

>**NOTE:** Refer Appendix 2 for summary of the interaction model
...
student_performance_intMdl2 = lm(G3 ~ (failures+higher+studytime+schoolsup+Dalc+health+romantic+famsize+
                                 ...
                                 data = studentDataset)
summary_intMdl2 = summary(student_performance_intMdl2)
tidy_fit_intMdl2 <- tidy(student_performance_intMdl2)
sigCoeff_intMdl2 = tidy_fit_intMdl2$term[tidy_fit_intMdl2$p.value<0.05]
sigCoeff_intMdl2
```

&nbsp;

>**NOTE:** Refer Appendix 3 for summary of final interaction model.
...
```{r eval=TRUE, echo=FALSE}
anova_addVsint = anova(student_performance_addMdl, student_performance_intMdl2)
anova_addVsint
```
...
```{r}
student_performance_hMdl1 = lm(G3 ~ (failures+I(failures^2)+higher+studytime+schoolsup+Dalc+health+romantic+famsize+
                                      +goout+failures:schoolsup+studytime:health+studytime:goout
                                      +schoolsup:health+schoolsup:goout), 
                                 data = studentDataset)
summary_hMdl1 = summary(student_performance_hMdl1)
tidy_fit_hMdl1 <- tidy(student_performance_hMdl1)
sigCoeff_hMdl1 = tidy_fit_hMdl1$term[tidy_fit_hMdl1$p.value<0.05]
sigCoeff_hMdl1
...
>Comment: Since the p-value is 0.3235 which is higher then our assumed $\alpha=0.05$ we cannot reject the $H_0$ and conclude that our model meets the assumption of common variance.
...
>Comment: Since the p-value is 2.2e-16 which is lower then our assumed $\alpha=0.05$ we should reject the $H_0$ and conclude that our model do not have normal distribution of error term and do not meet normality assumption.
...
### 2. Summary of interaction model: 
...
### 3. Summary of final interaction model:
...
### 3. Summary of higer order model:
...
```
```{r}
#fanyi <- function(model,dep) {
#    coe=coefficients(model)
#    for(a in names(coe)){
#      if (coe[a]>0){
#        print(paste('For one unit increases in ',a,' ',dep,' increases by ', signif(coe[a])))
#      }else{
#        print(paste('For one unit increases in ',a,' ',dep,' decreases by ', signif(-1*coe[a])))
#      }
#      
#    }
#} 
#fanyi(studentPerformance_fm,'the grade of student')
...
#  print(b)
#}
#formula(student_performance_intMdl1,'G3')
